...
// Lair expansion thresholds: level -> total lair space
const lairExpansionMap: Record<number, number> = {
	0: 12, // 3x4
	2: 15, // 3x5
	7: 20, // 4x5
	12: 24, // 4x6
...
export function getLairSpace(lv: number): number {
	const eligibleLevel = Object.keys(lairExpansionMap)
		.map(Number)
		.filter((lvl) => lvl <= lv)
		.sort((a, b) => b - a)[0];
	return lairExpansionMap[eligibleLevel] ?? 12;
}

export const lairSpace = $state({ total: 0, used: 0 });

$effect.root(() => {
	$effect(() => {
		const totalLairSpace = getLairSpace(level.value);
		const plannedLegendaries = Object.values(neededLegendaryCount).reduce((sum, v) => sum + v, 0);
		lairSpace.total = totalLairSpace;
		lairSpace.used = plannedLegendaries;
	});
});
